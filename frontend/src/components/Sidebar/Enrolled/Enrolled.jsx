...
import {
  BookOpen, Clock, Megaphone, X, Calendar,
  User, AlertCircle, Loader2, GraduationCap, ChevronRight,
} from 'lucide-react';
import { getStudentEnrolledShedule, getClassAnnouncements } from '../../../api/api';
            ...
            >
              ...
              <div className='enrolled-card-meta'>
                {cls.time_in_pakistan && (
                  <span><Clock size={13} />{cls.time_in_pakistan}:00 PKT</span>
                )}
                {cls.enrollment_date && (
                  <span><Calendar size={13} />Joined {formatDate(cls.enrollment_date)}</span>
                )}
              </div>
                ...
                <div>
                  ...
                  {selected.time_in_pakistan && (
                    <span className='drawer-schedule'>
                      <Clock size={13} />{selected.time_in_pakistan}:00 PKT
                    </span>
                  )}
                </div>
              </div>
                    ...
                    {announcements.map((ann) => (
                      <li key={ann.id} className='drawer-ann-item'>
                        ...
                        <span className='drawer-ann-author'>
                          <User size={12} />{ann.teacher_name ?? 'Instructor'}
                        </span>
                      </li>
                    ))}
                  </ul>
